...
function parse_options {
   ...
   if [ $# -lt 4 ] ; then
      echo "Expecting 4 positional arguments at least! Got $#: $*"
      print_help
      exit 1
   fi
   ...
   BASE_LINK=$3
   EEF_LINK=$4
   #Record the # of params aside for the "--" ones
   TOTAL_PARAM_CNT=$# 
   if [ $# -eq 4 ] ; then
      echo "No free joint index."
      return
   fi

   FREE_JOINT_1=$5
   if [ $# -eq 5 ] ; then
      echo "Has one free joint index."
      return
   fi

   FREE_JOINT_2=$6
   if [ $# -eq 6 ] ; then
      echo "Has two free joint index."
      return
   fi

   FREE_JOINT_3=$7
   if [ $# -eq 7 ] ; then
      echo "Has three free joint index."
      return
   fi

   FREE_JOINT_4=$8
   if [ $# -eq 8 ] ; then
      echo "Has four free joint index."
      return
   fi

   FREE_JOINT_5=$9
   if [ $# -eq 9 ] ; then
      echo "Has five free joint index."
      return
   fi

   FREE_JOINT_6=${10}
   if [ $# -eq 10 ] ; then
      echo "Has six free joint index."
      return
   fi
}
...
EOF
   ...
   if [ $TOTAL_PARAM_CNT -ge 5 ] ; then
      cmd+=" --freejoint=$FREE_JOINT_1"
   fi 
   if [ $TOTAL_PARAM_CNT -ge 6 ] ; then
      cmd+=" --freejoint=$FREE_JOINT_2"
   fi 
   if [ $TOTAL_PARAM_CNT -ge 7 ] ; then
      cmd+=" --freejoint=$FREE_JOINT_3"
   fi 
   if [ $TOTAL_PARAM_CNT -ge 8 ] ; then
      cmd+=" --freejoint=$FREE_JOINT_4"
   fi 
   if [ $TOTAL_PARAM_CNT -ge 9 ] ; then
      cmd+=" --freejoint=$FREE_JOINT_5"
   fi 
   if [ $TOTAL_PARAM_CNT -ge 10 ] ; then
      cmd+=" --freejoint=$FREE_JOINT_6"
   fi 
   echo "Running $cmd"
